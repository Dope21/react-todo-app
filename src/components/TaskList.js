          ...
          allTasks.map((task) => {
            return (
              <li 
                ...
                key={task.id}
              >
                  <div className='flex cursor-pointer select-none' onClick={()=> { setTaskState(task.id) }}>
                      { task.state ? checkBox :  unCheckBox }
                  </div>
                  <p className={
                        `font-light bg-transparent border-0 outline-none transition-all max-w-xl break-before-auto 
                  ...
                  <div 
                      className={
                        `bg-red-600 absolute px-5 py-2 cursor-pointer transition-all text-white 
                        ${delState ? 'visible right-0' : 'invisible right-[-100%]'}`
                      }
