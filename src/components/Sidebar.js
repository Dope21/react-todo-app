...
function Sidebar(props) {
    ...
    return (
        <nav className={'text-black bg-white dark:text-white dark:bg-purple-dark max-w-xs w-full fixed h-screen pt-20 px-7 pb-7 flex flex-col transition-all z-20 ' + props.navShow}>
            <div>
                <h2 className='text-3xl font-bold transition-all flex flex-col'>Welcome, {getUser() ? getUser().split('@')[0] : <span>TODO APP</span>}</h2>
                ...
                {
                    getUser() ?
                        <li onClick={handleLogout}><Link to="/" className='p-3 w-full bg-black bg-opacity-10 rounded-md flex items-center font-semibold transition-all hover:text-purple-main'><LogoutIcon className='w-6 mr-3 text-purple-main' />Logout</Link></li>
                        :
